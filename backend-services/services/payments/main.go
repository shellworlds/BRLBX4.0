// Payments service: Stripe subscriptions, meal payment records, carbon purchases, webhooks.
package main
...
import (
	...
	"github.com/shellworlds/BRLBX4.0/backend-services/services/payments/internal/api"
...
func main() {
	...
	dsn := config.MustGet("POSTGRES_PAYMENTS_DSN")
	...
	r := api.NewRouter(api.RouterConfig{
		...
		StripeWebhook:   config.GetString("STRIPE_WEBHOOK_SECRET"),
		DefaultPriceID:  config.GetString("STRIPE_DEFAULT_PRICE_ID"),
		InternalToken:   config.GetString("INTERNAL_PAYMENTS_TOKEN"),
	...
	go func() {
		sugar.Infow("payments listening", "addr", addr)
