...
func TestStore_Upsert_GetByAuth0(t *testing.T) {
	...
	mail := "a@b.c"
	mock.ExpectQuery(`INSERT INTO users`).WithArgs("sub1", mail, "vendor", pgxmock.AnyArg(), pgxmock.AnyArg()).
	...
	mock.ExpectQuery(`SELECT auth0_id`).WithArgs("sub1").WillReturnRows(
		pgxmock.NewRows([]string{"auth0_id", "email", "role", "client_id", "vendor_id", "updated_at"}).
			AddRow("sub1", mail, "vendor", nil, nil, time.Now()))
